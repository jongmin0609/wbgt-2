...
```text
https://wgbt-multi-worker-dashboard.onrender.com?view=worker&worker_id=W001
```
...
Start command: python -m streamlit run main.py --server.address=0.0.0.0 --server.port=$PORT --server.headless=true --server.enableCORS=false --server.enableXsrfProtection=false --browser.gatherUsageStats=false
...
Render 로그에 다음 오류가 반복되면 오래된 배포본의 Streamlit fragment 갱신 기능이
남아 있는 상태입니다.
...
2차 개선 앱에서는 이 문제를 피하기 위해 `st.experimental_fragment`를 사용하지
않습니다. GitHub에 최신 `main.py`를 올린 뒤 Render에서 `Manual Deploy`를 다시
실행하세요. 배포 후에도 같은 탭에서 오류가 계속 보이면 브라우저가 예전 세션을
잡고 있을 수 있으므로 탭을 닫고 새로 열거나 강력 새로고침을 합니다.
